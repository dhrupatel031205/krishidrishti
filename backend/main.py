...
_register_extra = {"requestBody": {"content": {"application/json": {"example": {
    "name": "Ramesh Patel", "email": "ramesh@example.com", "password": "farm1234",
    "phone": "9876543210", "farmName": "Patel Agro Farm", "location": "Ahmedabad, Gujarat"
}}}}}

@app.post("/api/auth/register", tags=["Auth"], openapi_extra=_register_extra)
...
_login_extra = {"requestBody": {"content": {"application/json": {"example": {
    "email": "ramesh@example.com", "password": "farm1234"
}}}}}

@app.post("/api/auth/login", tags=["Auth"], openapi_extra=_login_extra)
...
@app.get("/debug/plantnet", tags=["Core - Crop Disease Detection"])
def debug_plantnet():
    """## Debug – PlantNet API Connection Test
    Tests the PlantNet API key and connection using a tiny 1x1 pixel image.
    No image upload needed — just click Execute.

    **Returns:** key status, HTTP response code, and whether the API is reachable.
    """
    api_key = os.getenv("PLANTNET_API_KEY", "")
    info = {
        "key_configured": bool(api_key),
        "key_is_placeholder": api_key.startswith("your_") or not api_key,
        "key_preview": api_key[:8] + "..." if len(api_key) > 8 else "(empty)",
    }
    if not api_key or api_key.startswith("your_"):
        info["status"] = "SKIP — no valid key configured. Set PLANTNET_API_KEY in environment."
        return info
    # Send a minimal 1x1 white JPEG to test connectivity
    try:
        import struct, zlib
        ...
        r = requests.post(
            ...
            files=[("images", ("test.jpg", tiny_jpg, "image/jpeg"))],
            params={"api-key": api_key, "include-related-images": "false", "no-reject": "false", "lang": "en", "type": "kt"},
            timeout=12,
        )
        info["http_status"] = r.status_code
        info["response_preview"] = r.text[:400]
        if r.status_code == 200:
            info["status"] = "OK — API key is valid and PlantNet is reachable"
        elif r.status_code == 404:
            info["status"] = "OK — API reachable, key valid (404 = no plant found in test image, expected)"
        elif r.status_code == 401:
            info["status"] = "FAIL — 401 Unauthorized. Key is wrong or still domain-restricted."
        elif r.status_code == 429:
            info["status"] = "FAIL — 429 Rate limit exceeded (500/day free tier)."
        else:
            info["status"] = f"UNKNOWN — HTTP {r.status_code}"
    except Exception as e:
        info["status"] = f"EXCEPTION — {str(e)}"
    return info
...
_recommend_extra = {"requestBody": {"content": {"application/json": {"examples": {
    "Rice (high humidity)": {"summary": "Rice — high humidity, high rainfall",
        "value": {"N": 90, "P": 42, "K": 43, "temperature": 20.8, "humidity": 82.0, "ph": 6.5, "rainfall": 202.9}},
    "Chickpea (dry)": {"summary": "Chickpea — low water, dry soil",
        "value": {"N": 20, "P": 50, "K": 20, "temperature": 18.0, "humidity": 65.0, "ph": 7.0, "rainfall": 80.0}},
    "Maize": {"summary": "Maize — moderate conditions",
        "value": {"N": 60, "P": 55, "K": 44, "temperature": 22.0, "humidity": 70.0, "ph": 6.2, "rainfall": 120.0}},
}}}}}

@app.post("/recommend-crop", tags=["A - Crop Recommendation"], openapi_extra=_recommend_extra)
...
_irrigation_extra = {"requestBody": {"content": {"application/json": {"examples": {
    "Irrigate now": {"summary": "Irrigate now — dry soil, no rain",
        "value": {"soil_moisture": 32, "growth_stage": "vegetative", "rain_forecast_mm": 0, "temperature": 30}},
    "Delay — rain coming": {"summary": "Delay — rain forecast",
        "value": {"soil_moisture": 38, "growth_stage": "flowering", "rain_forecast_mm": 12, "temperature": 28}},
    "No action needed": {"summary": "No irrigation — soil is moist",
        "value": {"soil_moisture": 65, "growth_stage": "seedling", "rain_forecast_mm": 0, "temperature": 25}},
}}}}}

@app.post("/irrigation", tags=["B - Smart Irrigation"], openapi_extra=_irrigation_extra)
...
_sustain_extra = {"requestBody": {"content": {"application/json": {"examples": {
    "Efficient farm (High score)": {"summary": "Efficient farm — score ~95",
        "value": {"water_used_liters": 1400, "water_optimal_liters": 1400, "fertilizer_used_kg": 45, "fertilizer_recommended_kg": 45, "crop_health": 0.95}},
    "Inefficient farm (Low score)": {"summary": "Inefficient farm — score ~42",
        "value": {"water_used_liters": 3000, "water_optimal_liters": 1400, "fertilizer_used_kg": 90, "fertilizer_recommended_kg": 45, "crop_health": 0.60}},
    "Average farm": {"summary": "Average farm — score ~68",
        "value": {"water_used_liters": 1800, "water_optimal_liters": 1400, "fertilizer_used_kg": 55, "fertilizer_recommended_kg": 45, "crop_health": 0.80}},
}}}}}

@app.post("/sustainability-score", tags=["D - Sustainability Score"], openapi_extra=_sustain_extra)
...
_assistant_extra = {"requestBody": {"content": {"application/json": {"examples": {
    "English — disease query": {"summary": "English: tomato disease question",
        "value": {"message": "My tomato leaves have brown spots with yellow rings. What should I do?", "language": "en"}},
    "Hindi query": {"summary": "Hindi: pest problem",
        "value": {"message": "मेरी फसल में कीड़े लग गए हैं, क्या करूं?", "language": "hi"}},
    "Gujarati query": {"summary": "Gujarati: leaf spots",
        "value": {"message": "મારા ટામેટાના પાન પર ડાઘ છે, શું કરવું?", "language": "gu"}},
    "Disease context": {"summary": "Pass disease name directly",
        "value": {"disease": "Tomato Early Blight", "language": "en"}},
}}}}}

@app.post("/assistant", tags=["E - Farmer Assistant"], openapi_extra=_assistant_extra)
